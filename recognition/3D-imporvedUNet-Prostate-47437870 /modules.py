...
class DyReLU3d(nn.Module):
    ...
    def __init__(self, channels: int, reduction: int = 16, K: int = 2):
        ...
        self.fc2 = nn.Linear(hidden, 2 * K * channels)  # outputs [a1..aK, b1..bK] per channel
        # base offsets and ranges as in Dy-ReLU to stabilize learning
...
class ImprovedResidualBlock3d(nn.Module):
    """
    Improved residual block: BN -> DyReLU -> 3x3 -> BN -> DyReLU -> 3x3 with identity mapping.
    """
...
class UNet3DImproved(nn.Module):
    """
    Improved U-Net 3D with:
    - Improved residual encoder/decoder blocks using Dy-ReLU
