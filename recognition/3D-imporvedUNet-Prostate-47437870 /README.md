...
The aim is to achieve ≥ 0.70 Dice similarity coefficient (DSC) for all foreground classes, with a Improved UNet vairent. 

The secondary aim is to evaluate whether the improvments were necessary to achieve this goal.
...
* **Dropout:** Optional `Dropout3d` for light regularisation in small datasets.
...
### **Imporved Unet (Stress Test):**
![](pictures/first_Local.gif)
![](pictures/first_local_training.png)



## Full datset training for 10 epochs

Training against the full dataset was done with UQ rangpur compute with a scheduled job to a single a-100 GPU. Training took about 10 mins on this hardware. 
...
**Imporved Unet (Final Test)**
![](pictures/full_data_10_epochs.gif)
<img src="pictures/final_training_dice.png" alt="Final Training Dice" width="250">
<p align="center">
...
The average DSC dropped between the traditional to imporved UNET by 0.086 units. This result is not unexpected as often the Basis 3D UNet reaches its peak quickly but plateaus, while the improved one learns slower but achieves a higher final Dice after more epochs. This is indicated by the shape of the curves above but would need a new investgation to validate.  

In context of acheiving a DSC greater than 0.7, these results suggest the orrigional 3D unet is sufficent for this dataset.  
...
* **Epochs:** **100** by default with early exit. The assignment targets **≥ 0.70 DSC** so comand line arg specifying --epochs 10 is suficent
...
This is an intimentent model archetecture tested, but not continued. Similar to the final version, but batch indpendant by replacing batchnorm with FRN and TLU. The stress test on Improved UNet had a lot of static. [Improved U-Net3+ with Stage Residual for Brain Tumor Segmentation](https://pmc.ncbi.nlm.nih.gov/articles/PMC8793173/) discusses a similar issue comming from batch size dependance. It was not continued to keep changes from the orrigional 3D UNet small, keeping a tighter comparison of results. 
